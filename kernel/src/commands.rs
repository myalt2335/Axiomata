use crate::{cdmo, debug, console, time, serial, wait, history, memory, OS_NAME, OS_VERSION, fs, forth, editor, desktop};
...
static mut SCROLL_INVERTED: bool = true;
...
pub fn is_scroll_inverted() -> bool {
    unsafe { SCROLL_INVERTED }
}
...
fn toggle_scroll_inverted() -> bool {
    unsafe {
...
const SCROLL_USAGE: &str = "Usage: os scroll invert|normal|toggle|status";
const PERFORMANCE_USAGE: &str = "Usage: os performance winoutline move|resize";

fn os_usage() {
    console::write_line("Usage: os <font|cursor|hud|text|bg|clipboard|prmpth|scroll|performance> ...");
    ...
    console::write_line("  scroll invert|normal|toggle|status");
    console::write_line("  performance winoutline move|resize");
}
...
fn handle_scroll_args(args: &[&str]) -> Result<(), &'static str> {
    match args.get(0) {
        Some(cmd) if cmd.eq_ignore_ascii_case("invert") => {
            unsafe { SCROLL_INVERTED = true; }
            console::write_line("Scroll inverted.");
            Ok(())
        }
        Some(cmd) if cmd.eq_ignore_ascii_case("normal") => {
            unsafe { SCROLL_INVERTED = false; }
            console::write_line("Scroll set to normal.");
            Ok(())
        }
        Some(cmd) if cmd.eq_ignore_ascii_case("toggle") => {
            let inverted = toggle_scroll_inverted();
            if inverted {
                console::write_line("Scroll inverted.");
            } else {
                console::write_line("Scroll set to normal.");
...
fn handle_performance_args(args: &[&str]) -> Result<(), &'static str> {
    match (args.get(0), args.get(1)) {
        (Some(feature), Some(cmd))
            if feature.eq_ignore_ascii_case("winoutline")
                && cmd.eq_ignore_ascii_case("resize") =>
        {
            let enabled = desktop::toggle_resize_outline_enabled();
            if enabled {
                console::write_line("Window outline on resize enabled.");
            } else {
                console::write_line("Window outline on resize disabled.");
            }
            Ok(())
        }
        (Some(feature), Some(cmd))
            if feature.eq_ignore_ascii_case("winoutline")
                && cmd.eq_ignore_ascii_case("move") =>
        {
            let enabled = desktop::toggle_move_outline_enabled();
            if enabled {
                console::write_line("Window outline on move enabled.");
            } else {
                console::write_line("Window outline on move disabled.");
            }
            Ok(())
        }
        _ => Err(PERFORMANCE_USAGE),
    }
}
...
pub fn os_command(args: &[&str]) {
    ...
    match sub.as_str() {
        ...
        "scroll" => {
            if let Err(msg) = handle_scroll_args(&args[1..]) {
                console::write_line(msg);
            }
        }
        "performance" => {
            if let Err(msg) = handle_performance_args(&args[1..]) {
                console::write_line(msg);
            }
        }
        "text" => {
...
pub fn help(args: &[&str]) {
    if let Some(topic) = args.get(0) {
        ...
        let msg = match topic.as_str() {
            ...
            "os" => "Changes system settings (font, cursor, HUD, colors, scroll, cmdhistory, time, prompt, performance). Usage: os <subcommand> ...",
